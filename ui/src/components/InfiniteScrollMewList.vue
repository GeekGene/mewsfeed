<template>
  ...
  <BaseMewList
    ...
    :load-more-fn="loadMoreAndMarkDone"
...
const emit = defineEmits(["mew-pinned", "mew-unpinned", "load-more"]);

const { data, dataList, loading, loadingMore, error, mutate, loadMoreAsync } =
  useLoadMore(props.fetchFn, {
    //cacheKey: (params: any) => `${props.cacheKey}-${JSON.stringify(params)}`,

    isNoMore: (data) => data && data.noMore,
...
const loadMoreAndMarkDone = async (
  i: number,
  done: (stop: boolean) => void
) => {
  const res = await loadMoreAsync();
  done(res.noMore);
};
...
const updateFeedMew = async (feedMew: FeedMew) => {
  ...
  const index = data.value.list.findIndex((f: FeedMew) =>
...
const updateOriginal = async (response: FeedMew) => {
  ...
  const newData = { ...data.value };
  if (index !== -1) {
    mergeWith(newData.list[index], mergeNewData, (obj, src) => {
      if (isArray(obj)) {
