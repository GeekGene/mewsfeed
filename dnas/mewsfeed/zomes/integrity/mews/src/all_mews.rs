...
pub fn validate_create_link_all_mews(
  ...
  if EntryHash::from(base_address) != expected_base_address {
    return Ok(ValidateCallbackResult::Invalid("AllMews link must use 'all_mews' Path as its base".into()))
  }
...
pub fn validate_delete_link_all_mews(
  ...
  Ok(ValidateCallbackResult::Invalid(String::from("AllMews links cannot be deleted")))
}
