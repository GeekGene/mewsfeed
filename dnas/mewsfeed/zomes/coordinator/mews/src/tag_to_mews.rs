...
pub fn get_mew_hashes_for_tag(tag: String, link_type: impl LinkTypeFilterExt) -> ExternResult<Vec<ActionHash>> {
  ...
  let hashes: Vec<ActionHash> = links.iter().map(|l| ActionHash::from(l.target.clone())).collect();

  Ok(hashes)
}

pub fn get_mews_for_tag_with_context(tag: String, link_type: impl LinkTypeFilterExt) -> ExternResult<Vec<FeedMew>> {
