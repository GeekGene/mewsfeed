...
use mews_types::{Notification, NotificationType, MewType, Mew, Profile};
...
pub fn get_notifications_for_agent(agent: AgentPubKey) -> ExternResult<Vec<Notification>> {
  ...
  let mut n = make_notifications_for_records(mews_records, NotificationType::MyAgentMentioned, false)?; 
  notifications.append(&mut n);
  
  // Follows of my agent
  let followed_my_mews_linkdetails: LinkDetails = call_local_zome::<LinkDetails, AgentPubKey>("follows", "get_follower_link_details_for_creator", agent)?;

  let follow_create_actions = get_creates_from_linkdetails(vec!(followed_my_mews_linkdetails.clone()))?;
  let mut n: Vec<Notification> = make_notifications_for_createlinks(follow_create_actions, NotificationType::MyAgentFollowed, false)?;
  notifications.append(&mut n);

  // Unfollows of my agent
  let follow_delete_actions = get_deletes_from_linkdetails(vec!(followed_my_mews_linkdetails))?;
  let mut n: Vec<Notification> = make_notifications_for_deletelinks(follow_delete_actions, NotificationType::MyAgentUnfollowed, false)?;
  notifications.append(&mut n);

  // Pins of my Mews
  let pinned_my_mews_linkdetails: Vec<LinkDetails> = my_mews
    .iter()
    .map(|m| call_local_zome::<LinkDetails, ActionHash>("agent_pins", "get_pinner_link_details_for_hash", m.action_hashed().hash.clone()))
    .collect::<ExternResult<Vec<LinkDetails>>>()?;

  let pin_create_actions = get_creates_from_linkdetails(pinned_my_mews_linkdetails.clone())?;
  let mut n: Vec<Notification> = make_notifications_for_createlinks(pin_create_actions, NotificationType::MyMewPinned, true)?;
  notifications.append(&mut n);

  // Unpins of my mews
  let pin_delete_actions = get_deletes_from_linkdetails(pinned_my_mews_linkdetails)?;
  let mut n: Vec<Notification> = make_notifications_for_deletelinks(pin_delete_actions, NotificationType::MyMewUnpinned, true)?;
  notifications.append(&mut n);

  // Licks of my Mews
  let licked_my_mews_linkdetails: Vec<LinkDetails> = my_mews
    .iter()
    .map(|m| call_local_zome::<LinkDetails, ActionHash>("likes", "get_liker_link_details_for_hash", m.action_hashed().hash.clone()))
    .collect::<ExternResult<Vec<LinkDetails>>>()?;

  let lick_create_actions = get_creates_from_linkdetails(licked_my_mews_linkdetails.clone())?;
  let mut n: Vec<Notification> = make_notifications_for_createlinks(lick_create_actions, NotificationType::MyMewLicked, true)?;
  notifications.append(&mut n);

  // Unlicks of my mews
  let lick_delete_actions = get_deletes_from_linkdetails(licked_my_mews_linkdetails)?;
  let mut n: Vec<Notification> = make_notifications_for_deletelinks(lick_delete_actions, NotificationType::MyMewUnlicked, true)?;
  notifications.append(&mut n);

  // Responses to my Mews
  let mews_mentioning_me = my_mews
    .iter()
    .map(|m| get_responses_for_mew(GetResponsesForMewInput {
  ...
  let mut n = make_notifications_for_records(mews_mentioning_me, NotificationType::MyMewResponded, true)?; 
  notifications.append(&mut n);
  ...
  let mew_hashes_i_responded_to: Vec<ActionHash> = my_mews
    .iter()
    .filter_map(|record| {
      match record.entry().to_app_option::<Mew>().ok() {
        Some(Some(mew)) => {
          match mew.mew_type {
  ...
  let mews_responding_to_mews_i_responded_to: Vec<Record> = mew_hashes_i_responded_to
    .iter()
    .map(|ah| get_responses_for_mew(GetResponsesForMewInput {
  ...
  let mut n = make_notifications_for_records(mews_responding_to_mews_i_responded_to, NotificationType::FollowedYarnResponded, true)?; 
  notifications.append(&mut n);
...
fn get_agent_profile(agent_pub_key: AgentPubKey) -> ExternResult<Option<Profile>> {
  let maybe_agent_profile_record = call_local_zome::<Option<Record>, AgentPubKey>("profiles", "get_agent_profile", agent_pub_key)?;
...
fn get_creates_from_linkdetails(linkdetails: Vec<LinkDetails>) -> ExternResult<Vec<CreateLink>> {
  let create_actions: Vec<CreateLink> = linkdetails
    .iter()
    .map(|linkdetail| -> Vec<SignedHashed<Action>> {
      let linkdetails_inner = linkdetail.clone().into_inner();
    
      linkdetails_inner.iter().map(|l| l.clone().0).collect::<Vec<SignedHashed<Action>>>()
    })
    .flatten()
    .filter_map(|action_hashed| match action_hashed.action() {
...
fn make_notifications_for_createlinks(create_link_actions: Vec<CreateLink>, notification_type: NotificationType, include_feed_mew: bool) -> ExternResult<Vec<Notification>> {
  let notifications: Vec<Notification> = create_link_actions
    ...
    .map(|create_action| -> ExternResult<Notification> {
      ...
      let feed_mew = match include_feed_mew {
        true => Some(get_mew_with_context(ActionHash::from(create_action.base_address.clone()))?),
        false => None
      };
...
fn make_notifications_for_deletelinks(delete_link_actions: Vec<DeleteLink>, notification_type: NotificationType, include_feed_mew: bool) -> ExternResult<Vec<Notification>> {
  let notifications: Vec<Notification> = delete_link_actions
    .iter()
    .map(|delete_action| {
      let agent_profile = get_agent_profile(delete_action.author.clone())?;
      let feed_mew = match include_feed_mew {
        true => Some(get_mew_with_context(ActionHash::from(delete_action.base_address.clone()))?),
        false => None,
      };
...
fn make_notifications_for_records(records: Vec<Record>, notification_type: NotificationType, include_feed_mew: bool) -> ExternResult<Vec<Notification>> {
  records
    .iter()
