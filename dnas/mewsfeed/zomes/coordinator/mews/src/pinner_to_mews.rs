...
pub fn get_mews_for_pinner_with_context(agent: AgentPubKey) -> ExternResult<Vec<FeedMew>> {
    let pinned_hashes = call_local_zome::<Vec<AnyLinkableHash>, AgentPubKey>("agent_pins", "is_hash_pinned", agent)?;
...
pub fn get_pinner_links_for_hash(hash: ActionHash) -> ExternResult<Vec<Link>> {
    call_local_zome("agent_pins", "get_pinner_links_for_hash", hash)
}