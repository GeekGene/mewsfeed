...
// describe.concurrent("followed-creators-mews", () => {
...
it("Followed creators mews should include own mews", async () => {
  await runScenario(async (scenario) => {
    ...
    assert.ok(aliceMewsFeed.length === 1, "alice's mews feed includes her mew");
    assert.equal(aliceMewsFeed[0].mew.text, mewContent, "mew content matches");
  }, true);
});
