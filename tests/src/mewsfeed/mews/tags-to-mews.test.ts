...
// describe.concurrent("tags-to-mews", () => {
it("Hashtag, cashtag and mention", async () => {
  await runScenario(async (scenario) => {
    ...
    assert.equal(hashtaggedMews[0].mew.text, mewContent, "mew content matches");
    ...
    assert.ok(arabicHashtaggedMews.length === 1, "one mew with arabic hashtag");
    assert.equal(
